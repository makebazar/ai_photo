...
export function MediaViewer({
  ...
  const go = React.useCallback(
    (dir: -1 | 1) => onChangeIndex(clampIndex(safeIndex + dir, items.length)),
    [items.length, onChangeIndex, safeIndex],
  );
  ...
  const downloadCurrent = React.useCallback(async () => {
    ...
    setDownloading(true);
    try {
      const res = await fetch(active.url, { mode: "cors" });
      if (!res.ok) throw new Error("Bad response");
      const blob = await res.blob();
      const url = URL.createObjectURL(blob);
      const a = document.createElement("a");
      a.href = url;
      a.download = `promo-${active.id}.jpg`;
  ...
  }, [active?.id, active?.url, downloading]);
        ...
        >
          ...
          <div className="absolute right-3 top-3 z-20 flex gap-2">
            <button
              ...
              aria-label="Download"
              disabled={downloading || !active?.url}
            >
              <Download size={18} />
            </button>
          </div>
